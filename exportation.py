...
def estar_dentro(parche, anotacion):
    ...
    for j in anotacion_geo:
        if j.geom_type == 'Polygon':
            j_procesada = process_polygon(j)
...
def run(cyto_job, parameters):
    ...
    try:
        ...
        for parche in parches:
            ...
            output_path_detecciones = os.path.join(working_path, "output_detecciones.json")
            f = open(output_path_detecciones,"w+")
            json.dump(lista_detecciones, f)
            f.close()

            output_path_anotaciones = os.path.join(working_path, "output_anotaciones.json")
            f = open(output_path_anotaciones,"w+")
            json.dump(lista_anotaciones, f)
            f.close()

            #I save a file generated by this run into a "job data" that will be available in the UI. 
            job_data = JobData(job.id, "Generated File", "output_detecciones.json").save()
            job_data.upload(output_path_detecciones)

            job_data = JobData(job.id, "Generated File", "output_anotaciones.json").save()
            job_data.upload(output_path_anotaciones)
